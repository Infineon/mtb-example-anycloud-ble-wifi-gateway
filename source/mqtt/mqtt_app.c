...
*              the subscriber, publisher and led tasks. The task also handles
...
/* Flag to denote status of wifi and mqtt connection. */
bool wifi_connected = pdFALSE;
bool cloud_connected = pdFALSE;
...
void mqtt_client_task(void *pvParameters)
{
        ...
        if (pdTRUE == xQueueReceive(mqtt_task_q, &mqtt_status, portMAX_DELAY))
        {
            ...
            switch(mqtt_status)
            {
                ...
                case HANDLE_DISCONNECTION:
                {
                    ...
                    /* Notify the publisher task about the disconnection */
                    xTaskNotify(publisher_task_handle, 0u, eSetValueWithoutOverwrite);
     ...
     */
    exit_cleanup:
    printf("Terminating publisher and subscriber tasks...\r\n");
